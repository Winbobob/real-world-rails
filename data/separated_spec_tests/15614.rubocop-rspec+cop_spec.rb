...
RSpec.describe RuboCop::Cop::RSpec::Cop do
  ...
  context 'when the source path ends with `_spec.rb`' do
    ...
    it 'ignores the file if it is ignored' 
  ...
  context 'when the file is a source file without "spec" in the name' do
    it 'ignores the source when the path is not a spec file' 


    it 'ignores the source when the path is not a specified pattern' 
  ...
  context 'when custom patterns are specified' do
    ...
    it 'registers offenses when the path matches a custom specified pattern' 
