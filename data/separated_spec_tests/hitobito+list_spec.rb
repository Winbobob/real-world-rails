...
describe Export::Tabular::Events::List do
  ...
  context 'additional course labels' do
    ...
    context 'first row' do
      let(:row) { csv[0].split(';') }
      it 'should contain contain the additional course fields' 

    end

    context 'second row' do
      let(:row) { csv[1].split(';') }
      it 'should contain contain the additional course and record fields' 

    end

    context 'third row (course without record)' do
      let(:row) { csv[2].split(';') }
      it 'should contain the additional course fields' 
