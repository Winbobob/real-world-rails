describe 'My Dashboard My Classes card', :testui => true do
  ...
  if ENV["UI_TEST"] && Settings.ui_selenium.layer != 'production' && !Date.today.sunday?
    ...
    begin
        ...
        if user['classes']
          ...
          begin
            ...
            unless current_student_semester.nil?
              ...
              it "shows the current term for student UID #{uid}" 
              ...
              it "shows the enrolled course ids for UID #{uid}" 

              it "shows the enrolled course titles for UID #{uid}" 

              it "shows the wait list positions for UID #{uid}" 

              it "shows the enrolled course site names for UID #{uid}" 

              it "shows the enrolled course site descriptions for UID #{uid}" 
              ...
              student_classes.each do |course|
                ...
                if academics_api.multiple_primaries?(course)
                  academics_api.course_primary_sections(course).each do |prim_section|
                    ...
                    it "offers a link to the class page for #{api_course_title} for UID #{uid}" 
            ...
            unless current_teaching_semester.nil?
              ...
              it "shows the current term for teaching UID #{uid}" 
              ...
              it "shows the teaching course ids for UID #{uid}" 

              it "shows the teaching course titles for UID #{uid}" 

              it "shows the teaching course site names for UID #{uid}" 

              it "shows the teaching course site descriptions for UID #{uid}" 
            ...
            if other_sites.any?
              ...
              it "shows the 'other' course site names for UID #{uid}" 

              it "shows the 'other' course site descriptions for UID #{uid}" 
            ...
            if has_enrollments && has_teaching
              it "shows an Enrollments heading for UID #{uid}" 

              it "shows a Teaching heading for UID #{uid}" 

            elsif has_enrollments && !has_teaching && has_other_sites
              it "shows an Enrollments heading for UID #{uid}" 

              it "shows no Teaching heading for UID #{uid}" 

            elsif !has_enrollments && has_teaching && has_other_sites
              it "shows no Enrollments heading for UID #{uid}" 
            ...
            if has_other_sites
              it "shows an Other Site Memberships heading for UID #{uid}" 
            ...
            if current_student_semester.nil? && current_teaching_semester.nil? && academics_api.other_sites(current_term).nil?
              ...
              if status_api.is_student? && !status_api.is_faculty?
                ...
                it "shows a 'not enrolled' message for UID #{uid}" 

                it "offers a valid link to the Registrar site for UID #{uid}" 

                it "offers a valid link to the Cal Student Central site for UID #{uid}" 


              # Faculty with no teaching classes
              elsif status_api.is_faculty? && !status_api.is_student?
                it "shows a 'not teaching' message for UID #{uid}" 

                it "offers no link to the Registrar site for UID #{uid}" 

                it "offers no link to the Cal Student Central site for UID #{uid}" 
              ...
              # GSI with no student or teaching classes
                ...
                registrar_link_works = WebDriverUtils.verify_external_link(driver, my_classes.registrar_link_element, 'Welcome to our web site - Office Of The Registrar')
                cal_student_central_link_works = WebDriverUtils.verify_external_link(driver, my_classes.cal_student_central_link_element, 'Welcome! | Cal Student Central')
                it "shows a 'not enrolled and not teaching' message for UID #{uid}" 
      ...
      it 'has student or teaching classes for at least one of the test users' 

      it 'has Faculty Resources links for at least one of the test users' 
