    ...
    context 'scheme 9 claim' do
      ...
      context 'for trials' do
        ...
        context 'when no daily attendance uplift fees applied' do
          [0,1,3,nil].each do |trial_length|
            context "and claim has an actual trial length of #{trial_length || 'nil'}" do
              ...
              it "returns #{[trial_length, attendances_incl_in_basic_fee].compact.min} - as least of actual trial length or #{attendances_incl_in_basic_fee} (included in basic fee)" 
        ...
        context 'when daily attendance uplift fees applied' do
          ...
          it "returns sum of daily attendance fee types plus #{attendances_incl_in_basic_fee} (included in basic fee)" 
        ...
        context 'when no daily attendance uplift fees applied' do
          [0,1,3,nil].each do |trial_length|
            context "and claim has an actual retrial length of #{trial_length || 'nil'}" do
              ...
              it "returns #{[trial_length, attendances_incl_in_basic_fee].compact.min} - as least of actual retrial length or 2 (included in basic fee)" 
      ...
      context 'for retrials' do
        ...
        context 'when no daily attendance uplift fee (2+) applied' do
          [0,1,2,nil].each do |trial_length|
            context "and claim has an actual retrial length of #{trial_length || 'nil'}" do
              before { claim.update(retrial_actual_length: trial_length) }
              it "returns #{[trial_length, attendances_incl_in_basic_fee].compact.min} - as least of actual retrial length or #{attendances_incl_in_basic_fee} (included in basic fee)" 
