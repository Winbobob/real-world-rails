...
describe PG::Connection do
	...
	context "with async established connection", :socket_io do
		...
		it "conn.send_query and IO.select work" 

			res = wait_for_query_result(@conn2)
			expect( res.values ).to eq([["1"]])
		end

		it "conn.send_query and conn.block work" 
		...
		it "conn.async_query works" 


		it "can use conn.reset_start to restart the connection" 
		...
		it "should properly close a socket IO when GC'ed" 
	...
	describe "deprecated password encryption method" do
		it "can encrypt password for a given user" 


		it "raises an appropriate error if either of the required arguments is not valid" 

	end

	describe "password encryption method", :postgresql_10 do
		it "can encrypt without algorithm" 


		it "can encrypt with algorithm" 
	...
	it "discards previous results at #discard_results" 
	...
	it "discards previous results (if any) before waiting on #exec" 
	...
	it "calls the block if one is provided to #exec" 
	...
	it "can use conn.reset to restart the connection" 
	...
	context "multinationalization support", :ruby_19 do
		...
		describe "rubyforge #22925: m17n support" do
			...
			it "the connection should use JOHAB dummy encoding when it's set to JOHAB" 


			it "can retrieve server encoding as text" 


			it "can retrieve server encoding as ruby encoding" 
			...
			it "raises appropriate error if set_client_encoding is called with invalid arguments" 
		...
		describe "respect and convert character encoding of input strings" do
			...
			it "should convert query string and parameters to #send_query_params" 
		...
		it "rejects command strings with zero bytes" 
		...
		it "rejects query params with zero bytes" 

			expect{ @conn.send_query_prepared( "abc\x00", ["ab\x00"] ) }.to raise_error(ArgumentError, /null byte/)
		end

		it "rejects string with zero bytes in escape" 


		it "rejects string with zero bytes in escape_literal" 


		it "rejects string with zero bytes in escape_identifier" 


		it "rejects string with zero bytes in quote_ident" 


		it "rejects Array with string with zero bytes" 
	...
	describe "deprecated forms of methods" do
		it "should forward exec to exec_params" 


		it "should forward exec_params to exec" 


		it "should forward send_query to send_query_params" 

			expect(@conn.get_last_result.values).to eq( [["5"]] )
		end

		it "shouldn't forward send_query_params to send_query" 
