    ...
    context role do
      ...
      context 'in own group' do
        ...
        %w(create edit show update destroy).each do |action|
          it "may #{action} invoices in #{own_group}" 

        end

        %w(create edit show update destroy).each do |action|
          it "may #{action} articles in #{own_group}" 

        end

        [:reminder, :payment].each do |obj|
          it "may create #{obj} in #{own_group}" 

        end

        %w(edit show update).each do |action|
          it "may #{action} invoice_config in #{own_group}" 
      ...
      context 'in other group' do
        ...
        %w(create edit show update destroy).each do |action|
          it "may not #{action} invoices in #{other_group}" 

        end

        %w(create edit show update destroy).each do |action|
          it "may not #{action} articles in #{other_group}" 

        end

        [:reminder, :payment].each do |obj|
          it "may not create #{obj} in #{own_group}" 

        end

        %w(edit show update destroy).each do |action|
          it "may not #{action} invoice_config in #{other_group}" 
