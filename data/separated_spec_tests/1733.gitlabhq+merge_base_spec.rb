...
describe Gitlab::Git::MergeBase do
  ...
  describe '#sha' do
    context 'when the refs exist', :existing_refs do
      it 'returns the SHA of the merge base' 
    ...
    context 'when passing a missing ref', :missing_ref do
      it 'does not call merge_base on the repository but raises an error' 
    ...
    it 'returns a merge base when passing 2 branch names' 


    it 'returns a merge base when passing a tag name' 

  end

  describe '#commit' do
    context 'for existing refs with a merge base', :existing_refs  do
      it 'finds the commit for the merge base' 


      it 'only looks up the commit once' 
  ...
  describe '#unknown_refs', :missing_ref do
    it 'returns the the refs passed that are not part of the repository' 


    it 'only looks up the commits once' 
