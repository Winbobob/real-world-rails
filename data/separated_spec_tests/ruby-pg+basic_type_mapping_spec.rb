...
describe 'Basic type mapping' do
	...
	describe PG::BasicTypeMapForQueries do
		...
		it "should do bigdecimal param encoding" 


		it "should do IPAddr param encoding" 
	...
	describe PG::BasicTypeMapForResults do
		...
		describe "connection wide type mapping" do
			...
			it "should do text datetime without time zone type conversions" 


			[1, 0].each do |format|
				it "should convert format #{format} timestamps per TimestampUtc" 

				end
			end

			[1, 0].each do |format|
				it "should convert format #{format} timestamps per TimestampUtcToLocal" 

				end
			end

			[1, 0].each do |format|
				it "should convert format #{format} timestamps per TimestampLocal" 

				end
			end

			[0, 1].each do |format|
				it "should convert format #{format} timestamps with time zone" 
			...
			it "should do numeric type conversions" 
			...
			it "should do inet type conversions" 

				end
			end

			it "should do cidr type conversions" 
