...
module RSpec
  describe Support do
    ...
    describe '.method_handle_for(object, method_name)' do
      ...
      it 'fetches method definitions for vanilla objects' 


      it 'fetches method definitions for objects with method redefined' 


      it 'fetches method definitions for proxy objects' 
      ...
      context "for a BasicObject subclass", :if => RUBY_VERSION.to_f > 1.8 do
        ...
        it 'still works when Kernel has been mixed in' 
    ...
    describe '.class_of' do
      ...
      context 'with an object having a singleton class' do
        ...
        it 'returns its non-singleton ancestor class' 
    ...
    describe "failure notification" do
      ...
      it "defaults to raising the provided exception" 


      it "can be set to another callable" 


      it "isolates notifier changes to the current thread" 


      it "can be changed for the duration of a block" 


      it "resets the notifier back to what it originally was when the block completes, even if an error was raised" 
    ...
    describe "warning notification" do
      ...
      it "defaults to warning with the provided text" 
