  ...
  module Support
    describe OS do
      ...
      describe ".windows?" do
        %w[cygwin mswin mingw bccwin wince emx].each do |fragment|
          it "returns true when host os is #{fragment}" 

        end

        %w[darwin linux].each do |fragment|
          it "returns false when host os is #{fragment}" 

        end
      end

      describe ".windows_file_path?" do
        it "returns true when the file alt seperator is a colon" 


        it "returns false when file alt seperator is not present" 
    ...
    describe RubyFeatures do
      ...
      describe "#ripper_supported?" do
        ...
        it 'returns whether Ripper is correctly implemented in the current environment' 


        it 'does not load Ripper' 
