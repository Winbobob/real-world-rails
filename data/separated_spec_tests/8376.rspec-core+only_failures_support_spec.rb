  ...
  RSpec.describe Configuration, "--only-failures support" do
    ...
    describe "#last_run_statuses" do
      ...
      context "when `example_status_persistence_file_path` is configured" do
        ...
        it 'gets the last run statuses from the ExampleStatusPersister' 
      ...
      context "when `example_status_persistence_file_path` is not configured" do
        ...
        it 'returns a blank hash without attempting to load the persisted statuses' 
      ...
      it 'allows the value to be updated when `example_status_persistence_file_path` is set after first access' 


      it 'allows the value to be updated when `example_status_persistence_file_path` is forced after first access' 
    ...
    describe "#spec_files_with_failures" do
      ...
      context "when `example_status_persistence_file_path` is configured" do
        it 'returns a memoized array of unique spec files that contain failed exaples' 
      ...
      context 'when the file at `example_status_persistence_file_path` has corrupted `status` values' do
        ...
        it 'defaults invalid statuses to unknown' 

      end

      context "when `example_status_persistence_file_path` is not configured" do
        it "returns a memoized blank array" 
    ...
    describe "#files_to_run, when `only_failures` is set" do
      ...
      context "and no explicit paths have been set" do
        it 'loads only the files that have failures' 


        it 'loads the default path if there are no files with failures' 

      end

      context "and a path has been set" do
        it "loads the intersection of files matching the path and files with failures" 


        it "loads all files matching the path when there are no intersecting files" 
