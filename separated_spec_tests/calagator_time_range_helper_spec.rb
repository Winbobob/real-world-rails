...
describe TimeRangeHelper do
    ...
    describe "with#{context_date.nil? ? "out" : ""} context date" do
      [:text, :hcal, :html].each do |format|
        tests.each do |label, end_time, expected_without_context, expected_with_context|
          ...
          it "should format #{label} in #{format} format as '#{expected}'" 
  ...
  describe "with objects" do
    it "should format from objects that respond to just start_time" 


    it "should format from objects that respond to both start_time and end_time" 
