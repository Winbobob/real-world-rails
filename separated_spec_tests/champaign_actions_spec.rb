...
describe 'Api Actions' do
  ...
  describe 'POST#create' do
    ...
    describe 'Consent support' do
      ...
      it 'skips consent checks when no consent params are present' 


      context 'when `consent_enabled=false` or not present' do
        context 'and country is in the EEA' do
          it 'ignores `consented=true` param' 


          it 'ignores `consented=false` param' 
      ...
      context 'when `consent_enabled=true`' do
        context 'and country is in the EEA' do
          it 'creates action if `consented=true`' 


          it 'sets `consented_at` if `consented=true`' 


          it 'responds with success and a follow_up_url when `consented=true`' 


          it 'does not create an action when `consented=false`' 


          it 'responds with success and a follow_up_url when `consented=false`' 

        end

        context 'and country is not in the EEA' do
          it 'skips consent checks when `consented=false`' 
