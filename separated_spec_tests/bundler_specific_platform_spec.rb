...
RSpec.describe "bundle install with specific_platform enabled" do
  ...
  context "when on a darwin machine" do
    ...
    it "locks to both the specific darwin platform and ruby" 


    it "caches both the universal-darwin and ruby gems when --all-platforms is passed" 


    it "uses the platform-specific gem with extra dependencies" 


    context "when adding a platform via lock --add_platform" do
      it "adds the foreign platform" 


      it "falls back on plain ruby when that version doesnt have a platform-specific gem" 
