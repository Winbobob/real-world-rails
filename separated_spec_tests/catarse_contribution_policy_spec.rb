...
RSpec.describe ContributionPolicy do
  ...
  shared_examples_for 'update permissions' do
    ...
    it 'should deny access if user is not updating his contribution' 


    it 'should permit access if user is contribution owner' 
  ...
  shared_examples_for 'create permissions' do
    ...
    %w[draft deleted rejected successful failed waiting_funds].each do |state|
      it "should deny access if project is #{state}" 
