    ...
    context 'with membership' do
      ...
      context 'duplicated serial' do
        ...
        context 'matching email' do
          ...
          it 'returns existing bike if no_duplicate set' 

        end

        context 'non-matching email' do
          let(:email) { 'another_email@example.com' }
          it 'creates a bike for organization with v3_accessor' 

        end
      end

      it "doesn't create a bike without an organization with v3_accessor" 
