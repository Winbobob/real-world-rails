...
describe 'Checkout', type: :feature, inaccessible: true, js: true do
  ...
  context 'visitor makes checkout as guest without registration' do
    ...
    context 'defaults to use billing address' do
      ...
      it 'defaults checkbox to checked' 


      it 'remains checked when used and visitor steps back to address step' 
    ...
    context 'free shipping' do
      ...
      it "does not show 'Free Shipping' when there are no shipments" 
  ...
  context 'when several payment methods are available', js: true do
    ...
    it 'the first payment method should be selected' 


    it 'the fields for the other payment methods should be hidden' 
  ...
  context 'user has payment sources', js: true do
    ...
    it 'selects first source available and customer moves on' 


    it 'allows user to enter a new source' 
  ...
  context 'goes back from payment to add another item', js: true do
    ...
    it 'transit nicely through checkout steps again' 
  ...
  context 'from payment step customer goes back to cart', js: true do
    ...
    context 'and updates line item quantity and try to reach payment page' do
      ...
      it 'redirects user back to address step' 


      it 'updates shipments properly through step address -> delivery transitions' 
  ...
  context 'if coupon promotion, submits coupon along with payment', js: true do
    ...
    context 'the promotion makes order free (downgrade it total to 0.0)' do
      ...
      context 'user choose to pay by check' do
        it 'move user to complete checkout step' 
      ...
      context 'user choose to pay by card' do
        ...
        it 'move user to confirmation checkout step' 
  ...
  context 'order has only payment step' do
    ...
    it 'goes right payment step and place order just fine' 
  ...
  context 'when order is completed' do
    ...
    it 'does not display a thank you message on that order future visits' 
  ...
  context "order's address is outside the default included tax zone" do
    context 'so that no taxation applies to its product' do
      ...
      it 'correctly displays other product taxless price which has been added to cart later' 
  ...
  context 'user has store credits', js: true do
    ...
    context 'when not all Store Credits are used' do
      ...
      it 'page has data for (multiple) Store Credits' 


      it 'apply store credits button should move checkout to next step if amount is sufficient' 


      it 'apply store credits button should wait on payment step for other payment' 
      ...
      context 'remove store credits payments' do
        ...
        it 'remove store credits button should remove store_credits' 
    ...
    context 'when all Store Credits are used' do
      ...
      it 'page has no data for Store Credits when all Store Credits are used' 
