...
describe 'Background jobs ordering', reset: false do
  ...
  context 'Ordering ASTER data' do
    ...
    context 'before the order is processed' do
      it 'it shows the order in creating state' 
      ...
      context 'when the order queue is worked', pending_updates: true do
        ...
        it 'displays dropped granules that don\'t have the specified access method' 
  ...
  context "when submitting an order that will throw an error" do
    ...
    it 'initially shows an order in the "Creating" state' 
    ...
    context 'after the order throws an error' do
      ...
      it 'shows the order in the "Failed" state without reloading the page' 


      it 'shows an error code with information on reporting the error' 
