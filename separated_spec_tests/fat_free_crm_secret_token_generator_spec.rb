# frozen_string_literal: true

# Copyright (c) 2008-2014 Michael Dvorkin and contributors.
#
# Fat Free CRM is freely distributable under the terms of MIT license.
# See MIT-LICENSE file or http://www.opensource.org/licenses/mit-license.php
#------------------------------------------------------------------------------

require 'spec_helper'
require 'fat_free_crm/secret_token_generator'

describe FatFreeCRM::SecretTokenGenerator do
  let(:token) { 'e5a4b315c062dec4ecb40dabcde84fd6c067cb016a813702d2f4299ad16255c88ed1020bd47fb527e8e5f7052b04be1fbb8e63c043b8fb36f88d3c7d79a68681' }

  describe "setup!" do
    it "should not generate a new token if one exists" 


    it "should generate a token if none exists" 


    it "should generate a random token if not persisted" 

  end

  describe "token_exists?" do
    it "should be true" 


    it "should be false" 

  end

  describe "token" do
    it "should delegate to Setting" 

  end

  describe "new_token!" do
    it "should generate and set a new token" 

  end

  describe "generate_token!" do
    it "should generate a random token" 

  end
end

