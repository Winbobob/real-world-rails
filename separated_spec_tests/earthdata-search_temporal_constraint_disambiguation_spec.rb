...
describe "Temporal constraint disambiguation", reset: true do
  ...
  context 'when a temporal constraint is set and no date is focused' do
    ...
    context 'clicking the download button' do
      ...
      it 'takes the user to the access configuration page with the temporal constraint applied' 
  ...
  context 'when a date is focused and no temporal constraint is set', pending_updates: true do
    ...
    context 'clicking the download button' do
      ...
      it 'takes the user to the access configuration page for the granules on the selected date' 
  ...
  context 'a date is focused and and a temporal constraint is set' do
    ...
    context 'clicking the download button' do
      ...
      it 'presents the user with a dialog to choose temporal behavior' 


      it 'presents the user with a dialog with a close button' 


      context 'clicking the close button' do
        before(:each) { find("#choose-temporal-modal a.modal-close").click }

        it 'closes the temporal constraint modal' 
